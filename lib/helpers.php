...
function form_tag($name, $tag, $checked = false, $class = '')
{
    return '<label><input type="checkbox" name="'.$name.'" class="'.$class.'" value="'.escape($tag['id']).'" '.($checked ? 'checked="checked"' : '').'><span class="'.escape($tag['color']).'">'.escape($tag['name']).'</span></label>';
}
